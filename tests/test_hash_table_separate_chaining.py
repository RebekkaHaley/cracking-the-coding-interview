...
from ctci.hash_table_separate_chaining import SeparateChainingHashTable, Pair


@pytest.fixture
def hash_table():
    sample_table = SeparateChainingHashTable(capacity=100)
...
def test_should_create_hashtable():
    hash_table = SeparateChainingHashTable(capacity=100)
...
def test_should_create_empty_buckets():
    ...
    hash_table = SeparateChainingHashTable(capacity=expected_capacity)
...
def test_capacity_should_not_grow_when_adding_elements():
    expected_value = 100
    hash_table = SeparateChainingHashTable(capacity=expected_value)
...
def test_should_get_pairs_of_empty_hash_table():
    assert SeparateChainingHashTable(capacity=100).pairs == []
...
def test_should_get_values_of_empty_hash_table():
    assert SeparateChainingHashTable(capacity=100).values == []
...
def test_should_get_keys_of_empty_hash_table():
    assert SeparateChainingHashTable(capacity=100).keys == []
...
def test_should_not_create_hashtable_with_zero_capacity():
    with pytest.raises(ValueError):
        SeparateChainingHashTable(capacity=0)


def test_should_not_create_hashtable_with_negative_capacity():
    with pytest.raises(ValueError):
        SeparateChainingHashTable(capacity=-100)
...
def test_should_not_create_hashtable_with_negative_threshold():
    with pytest.raises(ValueError):
        SeparateChainingHashTable(load_factor_threshold=-100)
...
def test_should_report_capacity_of_empty_hash_table():
    assert SeparateChainingHashTable(capacity=100).capacity == 100
...
def test_should_have_canonical_string_representation(hash_table):
    assert repr(hash_table) in {
        "SeparateChainingHashTable.from_dict({'hello': 'world', 98.6: 37, False: True})",
        "SeparateChainingHashTable.from_dict({'hello': 'world', False: True, 98.6: 37})",
        "SeparateChainingHashTable.from_dict({98.6: 37, 'hello': 'world', False: True})",
        "SeparateChainingHashTable.from_dict({98.6: 37, False: True, 'hello': 'world'})",
        "SeparateChainingHashTable.from_dict({False: True, 'hello': 'world', 98.6: 37})",
        "SeparateChainingHashTable.from_dict({False: True, 98.6: 37, 'hello': 'world'})",
    }
...
def test_should_create_hashtable_from_dict():
    ...
    hash_table = SeparateChainingHashTable.from_dict(dictionary)
...
def test_should_create_hashtable_from_dict_with_custom_capacity():
    ...
    dictionary = {"hello": "world", 98.6: 37, False: True}
    hash_table = SeparateChainingHashTable.from_dict(dictionary, capacity=100)
    assert hash_table.capacity == expected_capacity
    assert hash_table.keys == list(dictionary.keys())
    assert hash_table.pairs == list(dictionary.items())
    assert unordered(hash_table.values) == list(dictionary.values())
...
def test_should_compare_equal_different_key_value_order(hash_table):
    h1 = SeparateChainingHashTable.from_dict({"a": 1, "b": 2, "c": 3})
    h2 = SeparateChainingHashTable.from_dict({"b": 2, "a": 1, "c": 3})
    assert h1 == h2


def test_should_compare_unequal(hash_table):
    other = SeparateChainingHashTable.from_dict({"different": "value"})
...
def test_should_compare_equal_different_capacity():
    data = {"a": 1, "b": 2, "c": 3}
    h1 = SeparateChainingHashTable.from_dict(data, capacity=50)
    h2 = SeparateChainingHashTable.from_dict(data, capacity=100)
    assert h1 == h2
...
def test_should_resize_hash_table_when_capacity_is_full():
    ...
    hash_table = SeparateChainingHashTable(capacity=1)
    for i in range(20):
        hash_table[i] = i
